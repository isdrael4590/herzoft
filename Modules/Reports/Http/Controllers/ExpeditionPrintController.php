...
class ExpeditionPrintController extends Controller
{
    ...
    {
        $items = Session::get('print_expedition_items');
        $timestamp = Session::get('print_expedition_timestamp');
        ...
        if ($timestamp && $timestamp->diffInMinutes(now()) > 30) {
            Session::forget(['print_expedition_items', 'print_expedition_timestamp']);
    ...
    {
        $chunks = Session::get('print_expedition_chunks');
        $totalItems = Session::get('print_expedition_total_items', 0);
        ...
        // Limpiar la sesión
        Session::forget(['print_expedition_chunks', 'print_expedition_total_items', 'print_expedition_timestamp']);
    ...
    private function generatePdf(array $items)
    {
        try {
            ...
            $expeditions = $this->getExpeditionsOptimized($items, $itemCount);

            if ($expeditions->isEmpty()) {
                throw new \Exception('No valid expeditions found for the provided IDs');
            }

            \Log::info("Found {$expeditions->count()} valid expeditions out of {$itemCount} requested");
            ...
            $totalPackages = $expeditions->sum(function ($expedition) {
                return $expedition->expeditionDetails ? $expedition->expeditionDetails->count() : 0;
            });

            $statusStats = $expeditions->groupBy('status_expedition')->map(function ($group) {
                return $group->count();
            });

            $areaStats = $expeditions->groupBy('area_expedition')->map(function ($group) {
                return $group->count();
            });
            ...
            $pdfData = [
                ...
                'selected_count' => $expeditions->count(),
            ...
            try {
                ...
                $pdf = PDF::loadView('reports::expedition.print', $pdfData)
                    ->setOptions($pdfOptions);
            }
    ...
    private function getExpeditionsOptimized(array $items, int $itemCount)
    {
        ...
        if ($itemCount > 5000) {
            ...
            foreach ($chunks as $chunkIndex => $chunk) {
                ...
                $chunkData = Expedition::with(['expeditionDetails' => function ($query) {
                    // Optimizar la carga de detalles si es necesario
                    $query->select('id', 'expedition_id', 'product_name'); // Solo campos necesarios
                }])
                    ->whereIn('id', $chunk)
                    ->get();

                $expeditions = $expeditions->merge($chunkData);
            }

            return $expeditions->sortByDesc('updated_at');
        } else {
            // Consulta normal para volúmenes menores
            return Expedition::with(['expeditionDetails' => function ($query) {
                $query->select('id', 'expedition_id', 'product_name');
            }])
                ->whereIn('id', $items)
    ...
    {
        // Usar vista existente o crear una básica para expedition
        // Ajustar según las vistas que tengas disponibles
        if ($itemCount > 10000) {
            return 'reports::expedition.print-expedition'; // Vista básica
        } elseif ($itemCount > 5000) {
            return 'reports::expedition.print-expedition'; // Vista básica
        } else {
            return 'reports::expedition.print-expedition'; // Vista principal
        }
    }

    private function generateFilename(int $itemCount)
    {
        return 'expedition-report-' . $itemCount . '-items-' . now()->format('Y-m-d-H-i-s') . '.pdf';
    }
