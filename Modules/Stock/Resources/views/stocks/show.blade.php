...
@section('title', 'Detalles Stock Generado')
    ...
    <div class="container-fluid mb-4">
        ...
        <div class="d-flex align-items-center justify-content-between mb-4">
            <div class="d-flex align-items-center">
                ...
                <div>
                    <h4 class="mb-0 font-weight-bold text-dark">Stock #{{ $stock->reference }}</h4>
                    <small class="text-muted">
                        {{ \Carbon\Carbon::parse($stock->created_up)->format('d M, Y') }}
                        &nbsp;&bull;&nbsp;
                        <span class="badge badge-info" style="font-size:.75rem;padding:4px 10px;border-radius:20px;">
                            <i class="bi bi-box-seam mr-1"></i> {{ $stock->machine_name }}
                        </span>
                    </small>
                </div>
            </div>
                        ...
                        @foreach ($items as $item)
                            <div class="d-flex justify-content-between align-items-center mb-2" style="font-size:.82rem;">
                                <span class="text-muted">
                                    <i class="bi {{ $item['icon'] }} mr-1" style="color:#0ea5e9;"></i>{{ $item['label'] }}
                                </span>
                    ...
                    <div class="card-body" style="padding:20px;">
                        <div class="d-flex align-items-start mb-3">
                            ...
                            <div>
                                ...
                                <p class="mb-0 font-weight-bold text-dark" style="font-size:.95rem;">{{ $stock->reference }}</p>
                            </div>
                        </div>
                        <div class="d-flex align-items-start mb-3">
                            <div class="rounded mr-3 d-flex align-items-center justify-content-center flex-shrink-0"
                        ...
                        <div class="d-flex align-items-start">
                            ...
                            <div>
                                ...
                                <p class="mb-0 font-weight-semibold text-dark" style="font-size:.875rem;">
                                    {{ $stock->stockDetails->count() }} {{ $stock->stockDetails->count() === 1 ? 'ítem' : 'ítems' }}
                                </p>
                            </div>
                        </div>
                    </div>
                        ...
                        <tbody>
                            @foreach ($stock->stockDetails as $item)
                                <tr style="border-color:#f1f5f9;">
                                    ...
                                    <td class="align-middle" style="padding:14px 12px;">
                                        @if ($item->product_status_stock == 'Disponible')
                                            <span class="badge badge-success" style="font-size:.75rem;padding:4px 10px;border-radius:20px;">
                                                <i class="bi bi-check-circle mr-1"></i>{{ $item->product_status_stock }}
                                            </span>
                                        @elseif ($item->product_status_stock == 'Despachado')
                                            <span class="badge badge-secondary" style="font-size:.75rem;padding:4px 10px;border-radius:20px;">
                                                <i class="bi bi-box-arrow-right mr-1"></i>{{ $item->product_status_stock }}
                                            </span>
                                        @else
