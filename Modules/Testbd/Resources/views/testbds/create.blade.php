...
@section('title', 'Nuevo Test Bowie & Dick')
...
@section('content')
    <div class="container-fluid">
        <form id="testbd-form" action="{{ route('testbds.store') }}" method="POST" enctype="multipart/form-data">
            @csrf
                        ...
                        <div class="col-md-3">
                            <div class="form-group">
                                ...
                                <select class="form-control" id="temp_machine" name="temp_machine" required
                                    style="border-radius:8px;border-color:#e2e8f0;padding:10px 14px;">
                                    <option value="134" {{ old('temp_machine', '134') == '134' ? 'selected' : '' }}>134 °C</option>
                                    <option value="121" {{ old('temp_machine') == '121' ? 'selected' : '' }}>121 °C</option>
                                </select>
                            </div>
                        </div>
                    </div>
                                    ...
                                    @foreach (\Modules\Informat\Entities\Informat::all() as $informat)
                                        @if ($informat->insumo_type == 'TEST BOWIE & DICK' && $informat->insumo_status == 'Activo')
                                            <option value="{{ $informat->insumo_lot }}"
                                                {{ old('lote_bd') == $informat->insumo_lot ? 'selected' : '' }}>
                                                {{ $informat->insumo_lot }}
                                            </option>
                                        @endif
                                    @endforeach
                        ...
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <select class="form-control" name="validation_bd" id="validation_bd" required
                                    ...
                                    <option value="Correcto" {{ old('validation_bd') == 'Correcto' ? 'selected' : '' }}>
                                        Correcto
                                    </option>
                                    <option value="Falla" {{ old('validation_bd') == 'Falla' ? 'selected' : '' }}>
                                        Falla
                                    </option>
                                </select>
                            </div>
