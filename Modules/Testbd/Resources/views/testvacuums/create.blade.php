...
@section('title', 'Nuevo Test Vacío')
...
@section('content')
    <div class="container-fluid">
        <form id="testvacuum-form" action="{{ route('testvacuums.store') }}" method="POST" enctype="multipart/form-data">
            @csrf
            ...
            <div class="d-flex align-items-center justify-content-between mb-4">
                <div class="d-flex align-items-center">
                    ...
                    <div>
                        <h4 class="mb-0 font-weight-bold text-dark">Nuevo Test Vacío</h4>
                        <small class="text-muted">Complete los campos para registrar el test de ciclo vacío</small>
                    </div>
                </div>
                                ...
                                <select class="form-control" id="machine_name" name="machine_name" required
                                    ...
                                    @foreach (\Modules\Informat\Entities\Machine::all() as $machine)
                                        <option value="{{ $machine->machine_name }}"
                                            {{ old('machine_name') == $machine->machine_name ? 'selected' : '' }}>
                        ...
                        <div class="col-md-4">
                            <div class="form-group">
                                ...
                                <select class="form-control" name="tipo_equipo" id="tipo_equipo" required
                                    ...
                                    <option value="Autoclave" {{ old('tipo_equipo') == 'Autoclave' ? 'selected' : '' }}>Autoclave</option>
                                    <option value="Peroxido" {{ old('tipo_equipo') == 'Peroxido' ? 'selected' : '' }}>Peróxido</option>
                                </select>
                            </div>
                        </div>
                        <div class="col-md-4">
                            <div class="form-group">
                                ...
                                <input type="number" class="form-control @error('lote_machine') is-invalid @enderror"
                                    name="lote_machine" required value="{{ old('lote_machine') }}"
                                    placeholder="Ej: 1"
                                    style="border-radius:8px;border-color:#e2e8f0;padding:10px 14px;">
                                @error('lote_machine')
                        ...
                        <div class="col-md-6">
                            <div class="form-group mb-0">
                                ...
                                <select class="form-control" name="validation_vacuum" id="validation_vacuum" required
                                    ...
                                    <option value="Correcto" {{ old('validation_vacuum') == 'Correcto' ? 'selected' : '' }}>Correcto</option>
                                    <option value="Falla" {{ old('validation_vacuum') == 'Falla' ? 'selected' : '' }}>Falla</option>
                                </select>
                            </div>
                        </div>
                    </div>
                            ...
                            <div class="form-group">
                                ...
                                <div class="input-group">
                                    <input type="number" class="form-control @error('temp_ambiente') is-invalid @enderror"
                                        name="temp_ambiente" required value="{{ old('temp_ambiente') }}"
                ...
                <div class="card-body" style="padding:24px;">
                    <div class="form-group mb-0">
                        ...
                        <textarea name="observation" id="observation" rows="4" class="form-control"
                            ...
                            style="border-radius:8px;border-color:#e2e8f0;padding:10px 14px;resize:vertical;">{{ old('observation') }}</textarea>
                    </div>
                </div>
            </div>
