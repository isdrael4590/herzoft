    ...
    <div class="container-fluid mb-4">
        ...
        <div class="d-flex align-items-center justify-content-between mb-4">
            <div class="d-flex align-items-center">
                ...
                <div>
                    <h4 class="mb-0 font-weight-bold text-dark">Lavado #{{ $lavado->reference }}</h4>
                    <small class="text-muted">
                        {{ \Carbon\Carbon::parse($lavado->created_at)->format('d M, Y H:i') }}
                        &nbsp;&bull;&nbsp;
                        @if ($lavado->status === 'Lavado')
                        ...
                        @elseif ($lavado->status === 'Observado')
                        ...
                        @elseif ($lavado->status === 'Procesado')
                        ...
                        @else
                            <span class="badge badge-secondary" style="font-size:.75rem;padding:4px 10px;border-radius:20px;">
                                <i class="bi bi-clock mr-1"></i> {{ $lavado->status }}
                            </span>
                        @endif
                    </small>
                </div>
            ...
            <div class="d-flex" style="gap:10px;">
                ...
                @if ($lavado->status !== 'Procesado')
                    <a href="{{ route('lavado-descarga.create', $lavado) }}"
                    ...
                    <a href="{{ route('lavado-preparations.create', $lavado) }}"
                        class="btn d-flex align-items-center text-white"
                        ...
                        @foreach ($items as $item)
                            <div class="d-flex justify-content-between align-items-center mb-2" style="font-size:.82rem;">
                                <span class="text-muted">
                                    <i class="bi {{ $item['icon'] }} mr-1" style="color:#f97316;"></i>{{ $item['label'] }}
                                </span>
                        ...
                        @foreach ($registros as $reg)
                            <div class="d-flex align-items-start mb-3">
                                <div class="rounded mr-3 d-flex align-items-center justify-content-center flex-shrink-0"
                                    ...
                                    <i class="bi {{ $reg['icon'] }} text-success" style="font-size:.9rem;"></i>
                                </div>
                                <div>
                        ...
                        @if ($lavado->note)
                            <div class="d-flex align-items-start">
                                ...
                                <div>
                                    ...
                                    <p class="mb-0 text-dark" style="font-size:.82rem;font-style:italic;">{{ $lavado->note }}</p>
                                </div>
                            </div>
                        @endif
                    </div>
                ...
                <div class="card border-0 h-100" style="border-radius:12px;box-shadow:0 2px 20px rgba(0,0,0,0.08);">
                    ...
                    <div class="card-body d-flex flex-column align-items-center justify-content-center" style="padding:20px;">
                        ...
                        <p class="text-muted mb-1 text-center" style="font-size:.8rem;">
                            <strong>{{ $lavado->reference }}</strong>
                        </p>
                        <p class="text-muted mb-0 text-center" style="font-size:.75rem;">
                            {{ \Carbon\Carbon::parse($lavado->created_at)->format('d M, Y') }}
                        </p>
                    </div>
                </div>
            </div>
        ...
        <div class="card border-0" style="border-radius:12px;box-shadow:0 2px 20px rgba(0,0,0,0.08);">
            <div class="card-header border-0 d-flex align-items-center justify-content-between"
                ...
                <span class="badge" style="font-size:.75rem;padding:5px 12px;border-radius:20px;background:rgba(249,115,22,0.12);color:#ea580c;">
                    {{ $lavado->lavadoDetalles->count() }} {{ $lavado->lavadoDetalles->count() === 1 ? 'ítem' : 'ítems' }}
                </span>
            </div>
                ...
                <div class="table-responsive">
                    <table class="table table-hover mb-0">
                        <thead style="background:#f8fafc;">
                            <tr>
                                @foreach (['Instrumental', 'Código', 'Cantidad', 'Paciente', 'Área', 'Tipo Proceso'] as $col)
                        ...
                        <tbody>
                            @forelse ($lavado->lavadoDetalles as $detalle)
                                <tr style="border-color:#f1f5f9;">
                                    <td class="align-middle" style="padding:14px 12px;">
                                        <span class="font-weight-semibold text-dark" style="font-size:.875rem;">{{ $detalle->product_name }}</span>
                                    </td>
                                    <td class="align-middle" style="padding:14px 12px;">
                                        <span class="badge badge-warning" style="font-size:.75rem;padding:4px 10px;border-radius:20px;">
