...
<?php if (app(\Illuminate\Contracts\Auth\Access\Gate::class)->check('edit_labelqrs')): ?>
    <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if(($data->status_cycle == 'Cargar' || $data->status_cycle == 'Pendiente') && ($data->machine_type == 'Autoclave')): ?>
...
<?php if (app(\Illuminate\Contracts\Auth\Access\Gate::class)->check('delete_labelqrs')): ?>
    <button class="dropdown-item"
        onclick="
            ...
            if (confirm('¿Está seguro? Desea eliminar permanentemente este proceso.')) {
