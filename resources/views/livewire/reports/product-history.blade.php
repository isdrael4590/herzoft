                    ...
                    <div class="d-flex justify-content-between align-items-center mb-3">
                        <div>
                            <span class="badge badge-info mr-2">
                                <i class="fas fa-list"></i> Total registros: {{ $totalRecords }}
                            </span>
                            <span class="badge badge-primary mr-2">
                                <i class="fas fa-cubes"></i> Cantidad total: {{ $totalQuantity }}
                            </span>
                        </div>
                            ...
                            <tbody>
                                @forelse ($history as $detail)
                                    <tr>
                                        ...
                                        <td>
                                            <span class="badge badge-secondary">{{ $detail->product_code }}</span>
                                        </td>
                                        <td>
                                        ...
                                        <td>{{ $detail->product_outside_company ?? '—' }}</td>
                                        <td>
                                            <span class="badge badge-primary">{{ $detail->product_quantity }}</span>
                                        </td>
                                        <td>{{ $detail->product_patient ?? '—' }}</td>
                                        <td>
                                            @if ($detail->product_type_dirt)
                                                @php
                                                    $dirtColors = [
                                                        'Bajo'     => 'success',
                                                        'Medio'    => 'warning',
                                                ...
                                                <span class="badge badge-{{ $color }}">
                                                    {{ $detail->product_type_dirt }}
                                                </span>
                                            @else
                                                —
                                            @endif
                                        </td>
                                        <td>{{ $detail->product_type_process ?? '—' }}</td>
                                        <td>{{ $detail->product_state_rumed ?? '—' }}</td>
                                        <td>{{ $detail->reception->operator ?? '—' }}</td>
                                        <td>
                                            @if ($detail->reception)
                                                @php
                                                    $statusColors = [
